...
![Release](https://img.shields.io/badge/Release-v1.3.8--beta--cn-brightgreen)
...
Release: [v1.3.8-beta-cn](https://github.com/Kuma1338/AlphaJong/releases/tag/v1.3.8-beta-cn)  
文件: `AlphaJong_1.3.8-beta-cn.user.js`

源码包内也已经包含可直接安装的 userscript：

[build/AlphaJong_1.3.8-beta-cn.user.js](./build/AlphaJong_1.3.8-beta-cn.user.js)
...
- 设置面板，支持手动输入参数和一键预设
- 策略记录窗口，可拖动并导出完整日志
- 牌局信息窗口，可查看剩余牌、对手弃牌、立直、鸣牌和危险摘要
- 保存每局结算后的名次 / 分数
- 导出策略记录 / 完整日志 / 对局结果 / BUG 数据
- 雀魂 Web 兼容性检查
- 三麻策略微调、名次策略和末盘追听牌逻辑
- 回归测试
- 发布包内置可安装 `.user.js`
...
| 悬浮位置 | 固定位置 | 可拖动，自动保存位置 |
| 语言 | 主要英文 | 用户层中文化 |
| 策略调参 | 修改源码常量 | 网页内设置面板，支持手动输入和预设，自动保存 |
| 决策复盘 | 主要看控制台日志 | 内置策略记录窗口和牌局信息窗口 |
| 对局统计 | 无 | 保存结算后的名次、分数和近期决策 |
| 数据导出 | 无 | 支持导出策略记录、完整日志、对局结果和 BUG 数据 |
...
2. 下载 Release 中的 `AlphaJong_1.3.8-beta-cn.user.js`。
...
| 记录 | 查看最近 20 次策略决策 |
| 牌局 | 查看当前牌局全局信息 |
| 参数预设 | 一键切换稳健防守、避四少放铳、快速和牌、三麻进攻 |
| 导出策略记录 | 导出最近决策和当前参数 |
| 导出完整日志 | 导出完整策略日志，便于复盘原因 |
| 导出对局结果 | 导出每局结算后的名次和分数 |
